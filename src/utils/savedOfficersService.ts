...
export const getCanonicalTherapistKey = (officerName: string = '', location: string = '', id: string = ''): string => {
  ...
  const full = `${nameLower} ${idLower}`;

  // Peralihan Siang
  if (full.includes('peralihan')) return 'box-peralihan-siang';
  if (full.includes('jemputan')) return 'box-jemputan';
  ...
  // Default fallback: cleaned name without degrees
